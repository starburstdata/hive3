 ...
 */
public class DecimalColGreaterEqualDecimalColumn extends VectorExpression {
  ...
  public DecimalColGreaterEqualDecimalColumn(int colNum1, int colNum2, int outputColumnNum) {
    super(outputColumnNum);
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
  }

  public DecimalColGreaterEqualDecimalColumn() {
     super();
  ...
  public void evaluate(VectorizedRowBatch batch) throws HiveException {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) {
      outputVector[0] = vector1[0].compareTo(vector2[0]) >= 0 ? 1 : 0;
    } else if (inputColVector1.isRepeating) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = vector1[0].compareTo(vector2[i]) >= 0 ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = vector1[0].compareTo(vector2[i]) >= 0 ? 1 : 0;
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = vector1[i].compareTo(value2) >= 0 ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = vector1[i].compareTo(value2) >= 0 ? 1 : 0;
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = vector1[i].compareTo(vector2[i]) >= 0 ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = vector1[i].compareTo(vector2[i]) >= 0 ? 1 : 0;
