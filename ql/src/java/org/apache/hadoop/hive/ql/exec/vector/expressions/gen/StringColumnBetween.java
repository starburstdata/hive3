...
public class StringColumnBetween extends VectorExpression {
  ...
  public StringColumnBetween(int colNum, byte[] left, byte[] right, int outputColumnNum) {
  ...
  public void evaluate(VectorizedRowBatch batch) throws HiveException {
    ...
    if (inputColVector.isRepeating) {
      if (inputColVector.noNulls || !inputIsNull[0]) {
        ...
        outputVector[0] =
            (StringExpr.compare(repeatBytes, repeatStart, repeatLength, left, 0, left.length) < 0 ||
      ...
      if (batch.selectedInUse) {
        ...
        if (!outputColVector.noNulls) {
          for(int j = 0; j != n; j++) {
           ...
           outputVector[i] =
               (StringExpr.compare(valueBytes, valueStart, valueLength, left, 0, left.length) < 0 ||
