 ...
 */
public class DecimalScalarLessEqualDecimalColumn extends VectorExpression {
  ...
  public DecimalScalarLessEqualDecimalColumn(HiveDecimal value, int colNum, int outputColumnNum) {
    super(outputColumnNum);
    this.value = new HiveDecimalWritable(value);
    this.colNum = colNum;
  }

  public DecimalScalarLessEqualDecimalColumn() {
     super();
  ...
  public void evaluate(VectorizedRowBatch batch) throws HiveException {
    ...
    if (inputColVector2.isRepeating) {
      if (inputColVector2.noNulls || !inputIsNull[0]) {
        ...
        outputVector[0] = value.compareTo(vector2[0]) <= 0 ? 1 : 0;
      } else {
        outputIsNull[0] = true;
      ...
      if (batch.selectedInUse) {
         ...
         if (!outputColVector.noNulls) {
           for(int j = 0; j != n; j++) {
             ...
             outputVector[i] = value.compareTo(vector2[i]) <= 0 ? 1 : 0;
          }
         } else {
           for(int j = 0; j != n; j++) {
             final int i = sel[j];
